...
public class BallCollisionsGoldenKerb : MonoBehaviour
{
    ...
    public void OnCollisionEnter2D(Collision2D collision)
    {
        ...
        if (collision.collider.CompareTag(roadTag) == true || collision.collider.CompareTag(road2Tag) == true)
        {
            ...
            if (!kerbHit)
            {
                ...
                roadHit = true;
                streak = 0;
                GetComponent<ScoreStreak>().currentStreak(streak);
                coinDisplay.GetComponent<CoinCount>().ResetJackpot();
                currentStreak.GetComponent<Text>().text = "Jackpot: " + coinDisplay.GetComponent<CoinCount>().jackpotTotal;
        ...
        if (collision.collider.CompareTag(roadTag) == true && kerbHit && !roadHit && !barrierHit)
        {
            ...
            streak++;
            GetComponent<ScoreStreak>().currentStreak(streak);
            coinDisplay.GetComponent<CoinCount>().AddToJackpot(streak);
            currentStreak.GetComponent<Text>().text = "Jackpot: " + coinDisplay.GetComponent<CoinCount>().jackpotTotal;
